...
func flatten(obj interface{}, key string) (map[string]interface{}, error) {
	//fmt.Printf("flatten.1 - key: %s, obj: %+v\n", key, obj)
	res := map[string]interface{}{}
	switch obj.(type) {
	case bool, int, int8, int16, int32, int64, uint, uint8, uint16, uint32, uint64:
		//fmt.Printf("case.bool_int: - key: %s, obj: %+v\n", key, obj)
		return map[string]interface{}{key: obj}, nil
	case float32, float64:
		//fmt.Printf("case.float: - key: %s, obj: %+v\n", key, obj)
		return map[string]interface{}{key: obj}, nil
	case string:
		//fmt.Printf("case.string: - key: %s, obj: %+v\n", key, obj)
		return map[string]interface{}{key: obj}, nil
	case []byte:
		//fmt.Printf("case.[]byte: - key: %s, obj: %+v\n", key, obj)
		return map[string]interface{}{key: obj}, nil
	case map[string]interface{}:
		//fmt.Printf("case.map: - key: %s, obj: %+v\n", key, obj)
		for k, v := range obj.(map[string]interface{}) {
			...
			nkey := k[:]
			if key != "" {
				nkey = fmt.Sprintf("%s.%s", key, k)
			}
			if flatten_value, err := flatten(v, nkey); err == nil {
				for fk, fv := range flatten_value {
					//fmt.Printf("fk: %s, fv: %v\n", fk, fv)
					res[fk] = fv
				}
			} else {
				return nil, err
	...
	case []interface{}:
		//fmt.Println("case []interface{}: key: ", key, " obj: ", obj)
		for idx, v := range obj.([]interface{}) {
			if key != "" {
				nkey := fmt.Sprintf("%s.[%d]", key, idx)
				//fmt.Printf("idx: %d, nkey: %v\n", idx, nkey)
				if flatten_value, err := flatten(v, nkey); err == nil {
					//					fmt.Printf("fv: %+v\n", flatten_value)
					for fk, fv := range flatten_value {
						//fmt.Printf("fk: %s, fv: %v\n", fk, fv)
	...
	default:
		// fmt.Printf("default:  type: %v, typeof: %v, obj: %v\n", reflect.TypeOf(obj), obj)
		return nil, nil
		//return map[string]interface{}{key: obj}, nil
	}
	return res, nil
...
func DotLookup(obj interface{}, key string) (interface{}, error) {
	...
	for _, x := range steps {
		...
		if prev_x != "" {
			_x := prev_x + x
			_x = strings.Replace(_x, "\\", ".", -1)
			switch xobj.(type) {
			case map[string]interface{}:
				xobj = xobj.(map[string]interface{})[_x]
			}
			prev_x = ""
		} else {
			switch xobj.(type) {
			case []interface{}:
				if len(x) >= 3 && x[0] == '[' && x[len(x)-1] == ']' {
					_x := x[1 : len(x)-1]
					if idx, err := strconv.Atoi(_x); err == nil {
						xobj = xobj.([]interface{})[idx]
					} else {
						return nil, err
					}
				} else {
					return nil, fmt.Errorf("cannot range over []interface{}")
				}
			case map[string]interface{}:
				xobj = xobj.(map[string]interface{})[x]
			}
		}
	}
	return xobj, nil
...
func DotLookupClone(obj interface{}, key string) (interface{}, error) {
	xobj, err := DotLookup(obj, key)
	// deep copy it to prevent further modification on original object.
	return deepcopy.Iface(xobj), err
}
