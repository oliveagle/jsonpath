...
var flatten_cases = []map[string]interface{}{
	...
	map[string]interface{}{
		// 6 {'a': ['b',1, {"c": 3}]}
		"obj":      map[string]interface{}{"a": []interface{}{"b", 1, map[string]interface{}{"c": 3}}},
		"query":    "a.[2].c",
		"expected": 3,
	},
	...
	map[string]interface{}{
		// 7 {'a': ['b',1, {"c": 3}]}
		"obj":      map[string]interface{}{"a.b.c": 1, "a": map[string]interface{}{"b": map[string]interface{}{"c": 2}}},
		"query":    "a.b.c",
		"expected": 2,
	},
	map[string]interface{}{
	...
	map[string]interface{}{
		// 9 {"a_a":{"b":1}}
		"obj":      map[string]interface{}{"a_a": map[string]interface{}{"b": 1.1}},
		"query":    "a_a.b",
		"expected": 1.1,
	},
}

var dot_lookup_cases = []map[string]interface{}{
	map[string]interface{}{
		// 0 {"a": 1}
		"obj":      map[string]interface{}{"a": 1},
		"query":    "a",
	...
	map[string]interface{}{
		...
		"obj":      map[string]interface{}{"a.b.c": 1, "a": map[string]interface{}{"b": map[string]interface{}{"c": 2}}},
		"query":    "a",
		"expected": map[string]interface{}{"b": map[string]interface{}{"c": 2}},
	},
}

func Test_jsonpath_flatten(t *testing.T) {
	for idx, tcase := range flatten_cases {
		obj := tcase["obj"]
		query := tcase["query"].(string)
		expected := tcase["expected"]

		fobj, err := flatten(obj, "")
		t.Log(idx, err, fobj)
		if err != nil {
			t.Errorf("flatten_cases: failed: [%d] %v", idx, err)
		}
		if got_obj, ok := fobj[query]; ok == false {
			t.Errorf("flatten_cases: query failed. didin't find %s in got_obj", query)
		} else {
			// t.Logf("typeof(got_obj): %v, typeof(expected): %v", reflect.TypeOf(got_obj), reflect.TypeOf(expected))
			if reflect.TypeOf(got_obj) != reflect.TypeOf(expected) {
				t.Errorf("flatten_cases: failed: type mismatch: %v", reflect.TypeOf(got_obj))
			}
			if got_obj != expected {
				t.Errorf("got_obj(%v) != expected(%v)", got_obj, expected)
			}
		}
	}
}

func Benchmark_jsonpath_flatten(b *testing.B) {
	obj := map[string]interface{}{"a.b.c": 1, "a": map[string]interface{}{"b": map[string]interface{}{"c": 2}}}
	for i := 0; i < b.N; i++ {
		flatten(obj, "")
	}
...
func Test_jsonpath_dot_lookup(t *testing.T) {

	for idx, tcase := range dot_lookup_cases {
		obj := tcase["obj"]
		query := tcase["query"].(string)
		expected := tcase["expected"]

		res, err := DotLookup(obj, query)
		t.Log(idx, err, res)
		if err != nil {
			t.Errorf("flatten_cases: failed: [%d] %v", idx, err)
		}
...
func Test_jsonpath_dot_lookup_modify(t *testing.T) {
	obj := map[string]interface{}{"a": []interface{}{
		1, "h", map[string]interface{}{"b": 2},
	}}

	t.Log(obj)

	xobj, _ := DotLookup(obj, "a")
	t.Log(obj, xobj)
	xobj = "x"
	t.Log(obj, xobj)
	...
	xobj.([]interface{})[0] = 2
	t.Log(obj, xobj)
	newv, _ := DotLookup(obj, "a.[0]")
	if newv != 2 {
		t.Error("nothing changed!!!: %v", obj)
	}
}

func Test_jsonpath_dot_lookup_clone_cannot_modify(t *testing.T) {
	obj := map[string]interface{}{"a": []interface{}{
		1, "h", map[string]interface{}{"b": 2},
	}}
	xobj, _ := DotLookupClone(obj, "a")
	t.Log(obj, xobj)
	xobj.([]interface{})[0] = 2
	t.Log(obj, xobj)
	newv, _ := DotLookup(obj, "a.[0]")
	if newv != 1 {
		t.Error("changed!!!: %v", obj)
	}
}

func Test_jsonpath_json_load(t *testing.T) {
	...
	fobj, _ := flatten(obj, "")
	fmt.Println(len(fobj))

	if len(fobj) != 298 {
		t.Error("len(fobj) != 298")
	}
	//	for k, v := range fobj {
	//		fmt.Println(">>> ", k, v)
	//	}
