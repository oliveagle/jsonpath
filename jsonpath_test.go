...
var parseToken_cases = []map[string]interface{}{
...
func Test_jsonpath_parseToken(t *testing.T) {
	for idx, tcase := range parseToken_cases {
...
func Test_jsonpath_getKey(t *testing.T) {
...
func Test_jsonpath_getIndex(t *testing.T) {
	obj := []interface{}{1, 2, 3, 4}
	res, err := getIndex(obj, 0)
	fmt.Println(err, res)
	if err != nil {
		t.Errorf("failed to getIndex(obj,0): %v", err)
		return
	}
	...
	res, err = getIndex(obj, -1)
	fmt.Println(err, res)
	if err != nil {
		t.Errorf("failed to getIndex(obj, -1): %v", err)
		return
	}
	...
	obj2 := []int{1, 2, 3, 4}
	res, err = getIndex(obj2, 0)
	fmt.Println(err, res)
	if err != nil {
		t.Errorf("failed to getIndex(obj2,0): %v", err)
		return
	}
	if v, ok := res.(int); ok != true || v != 1 {
		t.Errorf("failed to get int 1")
...
func Test_jsonpath_getRange(t *testing.T) {
	obj := []int{1, 2, 3, 4, 5}

	res, err := getRange(obj, 0, 2)
	fmt.Println(err, res)
	if err != nil {
		t.Errorf("failed to getRange: %v", err)
	}
	if res.([]int)[0] != 1 || res.([]int)[1] != 2 {
		t.Errorf("failed getRange: %v, expect: [1,2]", res)
	}

	obj1 := []interface{}{1, 2, 3, 4, 5}
	res, err = getRange(obj1, 3, -1)
	fmt.Println(err, res)
	if err != nil {
		t.Errorf("failed to getRange: %v", err)
	}
...
var tcase_parseFilter = []map[string]interface{}{
	// 0
	{
		"filter":  "@.isbn",
		"exp_lp":  "@.isbn",
...
func Test_jsonpath_parseFilter(t *testing.T) {

	//for _, tcase := range tcase_parseFilter[4:] {
	for _, tcase := range tcase_parseFilter {
...
var tcase_evalFilter = []map[string]interface{}{
	// 0
	{
		"obj":  map[string]interface{}{"a": 1},
...
func Test_jsonpath_evalFilter(t *testing.T) {
	for idx, tcase := range tcase_evalFilter[1:] {
		fmt.Println("------------------------------")
...
var tcase_cmpAny = []map[string]interface{}{
...
func Test_jsonpath_cmpAny(t *testing.T) {
	for idx, tcase := range tcase_cmpAny {
		//for idx, tcase := range tcase_cmpAny[8:] {
...
func BenchmarkJsonPathLookup_0(b *testing.B) {
	for i := 0; i < b.N; i++ {
		_, _ = JsonPathLookup(json_data, "$.expensive")
	}
}

func BenchmarkJsonPathLookup_1(b *testing.B) {
	for i := 0; i < b.N; i++ {
		_, _ = JsonPathLookup(json_data, "$.store.book[0].price")
	}
}

func BenchmarkJsonPathLookup_2(b *testing.B) {
	for i := 0; i < b.N; i++ {
		_, _ = JsonPathLookup(json_data, "$.store.book[-1].price")
	}
}

func BenchmarkJsonPathLookup_3(b *testing.B) {
	for i := 0; i < b.N; i++ {
		_, _ = JsonPathLookup(json_data, "$.store.book[0,1].price")
	}
}

func BenchmarkJsonPathLookup_4(b *testing.B) {
	for i := 0; i < b.N; i++ {
		_, _ = JsonPathLookup(json_data, "$.store.book[0:2].price")
	}
}

func BenchmarkJsonPathLookup_5(b *testing.B) {
	for i := 0; i < b.N; i++ {
		_, _ = JsonPathLookup(json_data, "$.store.book[?(@.isbn)].price")
	}
}

func BenchmarkJsonPathLookup_6(b *testing.B) {
	for i := 0; i < b.N; i++ {
		_, _ = JsonPathLookup(json_data, "$.store.book[?(@.price > 10)].title")
	}
}

func BenchmarkJsonPathLookup_7(b *testing.B) {
	for i := 0; i < b.N; i++ {
		_, _ = JsonPathLookup(json_data, "$.store.book[?(@.price < $.expensive)].price")
	}
}

func BenchmarkJsonPathLookup_8(b *testing.B) {
	for i := 0; i < b.N; i++ {
		_, _ = JsonPathLookup(json_data, "$.store.book[:].price")
	}
}

func BenchmarkJsonPathLookup_9(b *testing.B) {
	for i := 0; i < b.N; i++ {
		_, _ = JsonPathLookup(json_data, "$.store.book[?(@.author == 'Nigel Rees')].price")
	}
}

func BenchmarkJsonPathLookup_10(b *testing.B) {
	for i := 0; i < b.N; i++ {
		_, _ = JsonPathLookup(json_data, "$.store.book[?(@.author =~ /(?i).*REES/)].price")
	}
}
...
func TestRootArray(t *testing.T) {
	...
	_ = json.Unmarshal([]byte(booksData), &books)

	res, err := JsonPathLookup(books, "$[?(@.meta.language == 'en')]")
	if res_v, ok := res.([]interface{}); err != nil || ok == false || len(res_v) != 2 {
		fmt.Println(res, err)
		t.Fatal("root array support is broken")
	}

	res2, err := JsonPathLookup(books, "$[?(@.meta)]")
	if res_v, ok := res2.([]interface{}); err != nil || ok == false || len(res_v) != 2 {
		fmt.Println(res2, err)
		t.Fatal("root array support broken")
	}

	res3, err := JsonPathLookup(books, "$[*]")
	if res_v, ok := res3.([]interface{}); err != nil || ok == false || len(res_v) != 3 {
		fmt.Println(res3, err)
		t.Fatal("root array support broken")
	}

	res4, err := JsonPathLookup(books, "$[?(@.meta.language == 'en')].meta.release_year")
	if res_v, ok := res4.([]interface{}); err != nil || ok == false || len(res_v) != 2 {
