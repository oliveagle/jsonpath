...
// =============================================================================
// Tests for set_idx with nested navigation
// =============================================================================

// Test_set_idx_nested tests set_idx with nested steps
func Test_set_idx_nested(t *testing.T) {
	obj := map[string]interface{}{
		"items": []interface{}{
	...
	// Set items[1].name = "updated"
	s := step{op: "idx", key: "items", args: []int{1}}
	steps := []step{
		s,
		{op: "key", key: "name", args: nil},
	}
	res, err := set_idx(obj, s, steps, 0, "updated")
	if err != nil {
		t.Fatalf("set_idx failed: %v", err)
	}
	resMap := res.(map[string]interface{})
	...
	if item1["name"] != "updated" {
		t.Errorf("Expected name to be 'updated', got %v", item1["name"])
	}
}

// Test_set_idx_interface_unwrap tests set_idx with interface unwrap
func Test_set_idx_interface_unwrap(t *testing.T) {
	var iface interface{} = []interface{}{"a", "b", "c"}
	obj := map[string]interface{}{
		"items": iface,
	}
	s := step{op: "idx", key: "items", args: []int{0}}
...
// =============================================================================
// Tests for set_range with nested navigation
// =============================================================================

// Test_set_range_nested tests set_range with nested steps
func Test_set_range_nested(t *testing.T) {
	obj := []interface{}{
		map[string]interface{}{"name": "a"},
		map[string]interface{}{"name": "b"},
		map[string]interface{}{"name": "c"},
	}
	s := step{op: "range", args: [2]interface{}{0, 2}}
	steps := []step{
		s,
		{op: "key", key: "name", args: nil},
	}
	res, err := set_range(obj, s, steps, 0, "updated")
	if err != nil {
		t.Fatalf("set_range failed: %v", err)
	}
	resSlice := res.([]interface{})
	item0 := resSlice[0].(map[string]interface{})
	item1 := resSlice[1].(map[string]interface{})
	if item0["name"] != "updated" {
		t.Errorf("Expected item 0 name to be 'updated', got %v", item0["name"])
	}
	if item1["name"] != "updated" {
		t.Errorf("Expected item 1 name to be 'updated', got %v", item1["name"])
	}
}
...
// Test_set_key_nested tests set_key with nested navigation
func Test_set_key_nested(t *testing.T) {
	obj := map[string]interface{}{
		"a": map[string]interface{}{
			"b": map[string]interface{}{
				"c": 1,
	...
	s := step{op: "key", key: "a", args: nil}
	steps := []step{
		s,
		{op: "key", key: "b", args: nil},
		{op: "key", key: "c", args: nil},
	}
	res, err := set_key(obj, "a", steps, 0, 99)
	if err != nil {
		t.Fatalf("set_key failed: %v", err)
	}
	resMap := res.(map[string]interface{})
	a := resMap["a"].(map[string]interface{})
	b := a["b"].(map[string]interface{})
...
// Test_set_key_invalid_type tests set_key with invalid type
func Test_set_key_invalid_type(t *testing.T) {
	obj := "string"
	_, err := set_key(obj, "key", []step{}, 0, "value")
	if err == nil {
		t.Error("Expected error for invalid type")
	}
}
...
// =============================================================================
// Tests for get_range with more branches
// =============================================================================

// Test_get_range_from_nil tests get_range with nil from
func Test_get_range_from_nil(t *testing.T) {
	obj := []interface{}{1, 2, 3, 4, 5}
	...
	resSlice := res.([]interface{})
	// from=nil means 0, to=2 means [0:3] (to is exclusive+1)
	t.Logf("Got %d elements: %v", len(resSlice), resSlice)
}

// Test_get_range_both_nil tests get_range with both nil (wildcard)
func Test_get_range_both_nil(t *testing.T) {
	obj := []interface{}{1, 2, 3}
...
// Test_Lookup_uncovered_paths tests uncovered branches in Lookup function
func Test_Lookup_uncovered_paths(t *testing.T) {
	// Test: range with invalid args type (line 134-136)
	t.Run("lookup_range_invalid_args", func(t *testing.T) {
		c := &Compiled{
			path: "$.test",
			steps: []step{
				{op: "range", key: "", args: "invalid"}, // args should be [2]interface{}
			},
		}
		obj := map[string]interface{}{"test": []interface{}{1, 2, 3}}
		_, err := c.Lookup(obj)
		if err == nil {
			t.Error("Expected error for invalid range args type")
		}
	})

	// Test: idx with empty args slice (line 117-120)
	t.Run("lookup_idx_empty_args", func(t *testing.T) {
		c := &Compiled{
			path: "$[[]]",
			steps: []step{
				{op: "idx", key: "", args: []int{}}, // empty args
			},
		}
		obj := []interface{}{1, 2, 3}
		_, err := c.Lookup(obj)
		if err == nil {
			t.Error("Expected error for empty index args")
		}
	})

	// Test: default case with unknown op (line 183-185)
	t.Run("lookup_unknown_operation", func(t *testing.T) {
		c := &Compiled{
			path: "$.unknown",
			steps: []step{
				{op: "unknown_op", key: "test", args: nil},
			},
		}
		obj := map[string]interface{}{"test": 1}
		_, err := c.Lookup(obj)
		if err == nil {
	...
	// Test: idx with key error propagation (line 94-97)
	t.Run("lookup_idx_key_error", func(t *testing.T) {
		c := &Compiled{
			path: "$.invalid[0]",
			steps: []step{
				{op: "idx", key: "invalid", args: []int{0}},
			},
		}
		obj := map[string]interface{}{"other": []interface{}{1, 2, 3}}
		_, err := c.Lookup(obj)
		if err == nil {
	...
	// Test: range with key error propagation (line 124-127)
	t.Run("lookup_range_key_error", func(t *testing.T) {
		c := &Compiled{
			path: "$.invalid[0:1]",
			steps: []step{
				{op: "range", key: "invalid", args: [2]interface{}{0, 1}},
			},
		}
		obj := map[string]interface{}{"other": []interface{}{1, 2, 3}}
		_, err := c.Lookup(obj)
		if err == nil {
			t.Error("Expected error for non-existent key in range")
		}
	})

	// Test: filter with key error propagation (line 138-141)
	t.Run("lookup_filter_key_error", func(t *testing.T) {
		c := &Compiled{
			path: "$.invalid[?(@ > 1)]",
			steps: []step{
				{op: "filter", key: "invalid", args: "@ > 1"},
			},
		}
		obj := map[string]interface{}{"other": []interface{}{1, 2, 3}}
		_, err := c.Lookup(obj)
		if err == nil {
			t.Error("Expected error for non-existent key in filter")
		}
	})

	// Test: func with error propagation (line 172-175)
	t.Run("lookup_func_error", func(t *testing.T) {
		c := &Compiled{
			path: "$.invalid_func()",
			steps: []step{
				{op: "func", key: "invalid_func", args: nil},
			},
		}
		obj := map[string]interface{}{}
		_, err := c.Lookup(obj)
...
// Test_Lookup_idx_on_slice tests idx operation on slice types
func Test_Lookup_idx_on_slice(t *testing.T) {
	// Test: idx with multiple indices (line 100-110)
	t.Run("lookup_idx_multiple", func(t *testing.T) {
		c := &Compiled{
			path: "$[0,2]",
			steps: []step{
				{op: "idx", key: "", args: []int{0, 2}},
			},
		}
		obj := []interface{}{"a", "b", "c", "d"}
		res, err := c.Lookup(obj)
		if err != nil {
			t.Fatalf("Lookup failed: %v", err)
		}
		resSlice := res.([]interface{})
		if len(resSlice) != 2 || resSlice[0] != "a" || resSlice[1] != "c" {
			t.Errorf("Expected [a, c], got %v", resSlice)
		}
	})

	// Test: idx on slice with key (line 92-98)
	t.Run("lookup_idx_with_key", func(t *testing.T) {
		c := &Compiled{
			path: "$.items[0]",
			steps: []step{
				{op: "idx", key: "items", args: []int{0}},
			},
		}
		obj := map[string]interface{}{
...
// Test_Lookup_recursive tests recursive operation coverage
func Test_Lookup_recursive(t *testing.T) {
	// Test: recursive with non-key next step (line 146-163)
	t.Run("lookup_recursive_non_key_next", func(t *testing.T) {
		c := &Compiled{
			path: "$..*",
			steps: []step{
				{op: "recursive", key: "..", args: nil},
			},
		}
		obj := map[string]interface{}{
			"a": map[string]interface{}{
		...
		if !ok || len(resSlice) == 0 {
			t.Errorf("Expected non-empty slice, got %v", res)
		}
	})

	// Test: recursive followed by idx (not key)
	t.Run("lookup_recursive_then_idx", func(t *testing.T) {
		c := &Compiled{
			path: "$..[0]",
			steps: []step{
				{op: "recursive", key: "..", args: nil},
				{op: "idx", key: "", args: []int{0}},
			},
		}
		obj := map[string]interface{}{
			"items": []interface{}{
...
// Test_cmp_any tests cmp_any function coverage
func Test_cmp_any(t *testing.T) {
	// Test: invalid operator
	t.Run("cmp_any_invalid_op", func(t *testing.T) {
	...
	t.Run("cmp_any_string_ops", func(t *testing.T) {
		tests := []struct {
			op       string
			expected bool
		}{
		...
		for _, tt := range tests {
			result, err := cmp_any("a", "b", tt.op)
			if err != nil {
				t.Fatalf("cmp_any failed for op %s: %v", tt.op, err)
			}
			if result != tt.expected {
				t.Errorf("cmp_any(\"a\", \"b\", %s) = %v, expected %v", tt.op, result, tt.expected)
			}
		}
	})

	// Test: number comparison
	t.Run("cmp_any_numbers", func(t *testing.T) {
		tests := []struct {
			obj1     interface{}
			obj2     interface{}
			op       string
			...
			{2, 1, ">=", true},
			{1.5, 2.5, "<", true},
			{float64(10), float64(10), "==", true},
		}
		for _, tt := range tests {
			result, err := cmp_any(tt.obj1, tt.obj2, tt.op)
			if err != nil {
				t.Fatalf("cmp_any failed for %v %s %v: %v", tt.obj1, tt.op, tt.obj2, err)
			}
			if result != tt.expected {
				t.Errorf("cmp_any(%v, %v, %s) = %v, expected %v", tt.obj1, tt.obj2, tt.op, result, tt.expected)
			}
		}
	})

	// Test: mixed types (string vs number)
	t.Run("cmp_any_mixed_types", func(t *testing.T) {
		// This tests the string comparison path where both are not numbers
		result, err := cmp_any("10", 5, ">")
		if err != nil {
			// Error is expected since comparing string with number
			t.Logf("Expected error for mixed types: %v", err)
		} else {
			t.Logf("Result for mixed types: %v", result)
		}
	})
}

// Test_eval_match tests eval_match function coverage
func Test_eval_match(t *testing.T) {
	// Test: invalid regex pattern
	t.Run("eval_match_invalid_regex", func(t *testing.T) {
		result, err := eval_match(nil, nil, []string{"test", "[invalid"})
		if err == nil {
			t.Error("Expected error for invalid regex")
		}
		if result != nil {
	...
	// Test: valid patterns
	t.Run("eval_match_valid_patterns", func(t *testing.T) {
		tests := []struct {
			input    string
			pattern  string
			expected bool
			...
			{"hello", "hello", true},
			{"hello", "^he", false},        // anchored, should not match
			{"hello", ".*ell.*", true},     // with wildcards
			{"Hello", "(?i)hello", true},   // case insensitive - (?i) flag makes it match
			{"Hello", "(?i)^Hello$", true}, // exact match with flags
			{"", "^$", true},               // empty string
			{"test", ".*", true},           // match anything
		}
		for _, tt := range tests {
			result, err := eval_match(nil, nil, []string{tt.input, tt.pattern})
			if err != nil {
				t.Fatalf("eval_match failed for %s with pattern %s: %v", tt.input, tt.pattern, err)
			}
			if result != tt.expected {
				t.Errorf("eval_match(%s, %s) = %v, expected %v", tt.input, tt.pattern, result, tt.expected)
			}
		}
	})

	// Test: with @. prefix
	t.Run("eval_match_with_at_prefix", func(t *testing.T) {
		obj := map[string]interface{}{"name": "test"}
		result, err := eval_match(obj, nil, []string{"@.name", "test"})
		if err != nil {
			t.Fatalf("eval_match failed: %v", err)
		}
		if result != true {
...
// Test_eval_search tests eval_search function coverage
func Test_eval_search(t *testing.T) {
	// Test: invalid regex pattern
	t.Run("eval_search_invalid_regex", func(t *testing.T) {
		result, err := eval_search(nil, nil, []string{"test", "[invalid"})
		if err == nil {
			t.Error("Expected error for invalid regex")
		}
		if result != nil {
	...
	// Test: valid patterns (search without anchoring)
	t.Run("eval_search_valid_patterns", func(t *testing.T) {
		tests := []struct {
			input    string
			pattern  string
			expected bool
			...
			{"hello world", "^world", false}, // anchored at start, won't match
			{"hello world", "wor", true},     // partial match
			{"Hello", "(?i)HELLO", true},     // case insensitive search
			{"", ".*", true},                 // empty string matches .*
			{"test", "^t", true},             // starts with t
			{"test", "t$", true},             // ends with t
		}
		for _, tt := range tests {
			result, err := eval_search(nil, nil, []string{tt.input, tt.pattern})
			if err != nil {
				t.Fatalf("eval_search failed for %s with pattern %s: %v", tt.input, tt.pattern, err)
			}
			if result != tt.expected {
				t.Errorf("eval_search(%s, %s) = %v, expected %v", tt.input, tt.pattern, result, tt.expected)
			}
		}
	})
}

// Test_eval_filter tests eval_filter function coverage
func Test_eval_filter(t *testing.T) {
	// Test: expression that doesn't evaluate to boolean (exists check on nil)
	t.Run("eval_filter_exists_nil", func(t *testing.T) {
		obj := map[string]interface{}{"a": 1}
		// exists check with nil value
		result, err := eval_filter(obj, obj, "@.nonexistent", "", "")
		if err != nil {
			t.Fatalf("eval_filter failed: %v", err)
		}
		if result != false {
	...
	t.Run("eval_filter_valid_expr", func(t *testing.T) {
		obj := map[string]interface{}{
			"price": 10.0,
			"name":  "test",
		}
		...
		for _, tt := range tests {
			result, err := eval_filter(obj, obj, tt.lp, tt.op, tt.rp)
			if err != nil {
				t.Fatalf("eval_filter failed for %s %s %s: %v", tt.lp, tt.op, tt.rp, err)
			}
			if result != tt.expected {
				t.Errorf("eval_filter(%s %s %s) = %v, expected %v", tt.lp, tt.op, tt.rp, result, tt.expected)
			}
		}
	})
}
